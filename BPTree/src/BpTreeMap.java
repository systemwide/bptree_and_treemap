       ...
       implements Serializable, Cloneable, SortedMap <K, V>
{
    ...
    public Set <Map.Entry <K, V>> entrySet ()
    {
        ...
        // Start at the first leaf
 		...
 		while(!currNode.isLeaf) {
 			currNode = (Node) currNode.ref[0];
 		}
        
        // Keep adding more entries until the node is full
        while(currNode != null) {
        	// Go through current node
        	for(int i = 0; i < currNode.nKeys; i++) {
        		enSet.add(new SimpleEntry<K, V>(currNode.key[i], (V) currNode.ref[i]));
        	}
        	
        	// Get the next linked leaf
        	currNode = (Node) currNode.ref[ORDER - 1];
        }
    ...
    public K lastKey () 
    {
    	...
    	// Keep track if it's the first node checked or not
		...
		for(Map.Entry<K, V> item : entrySet()) {
			// Get the key from the set
			K key = item.getKey();
			
			// Initialize the largestNode on the first node (ran only once)
			if(isFirstRun) {
				largestNode = key;
				isFirstRun = false;
			} 
			
			else { // Check whether current the key is larger than the previously largest node
				if(key.compareTo(largestNode) > 0) {
					largestNode = key;
				}
			}
		}
    ...
    public SortedMap <K,V> tailMap (K fromKey)
    {
    	...
    	SortedMap<K,V> tailMap = new TreeMap<>();
		...
		while(currNode != null) {
			// Go through current node
			for(int i = 0; i < currNode.nKeys; i++) {
				if(fromKey.compareTo(currNode.key[i]) <= 0) {
					tailMap.put(currNode.key[i], (V) currNode.ref[i]);
				}
			}
    ...
    public SortedMap <K,V> subMap (K fromKey, K toKey)
    {
		...
		while(currNode != null) {
			...
			for(int i = 0; i < currNode.nKeys; i++) {
				if(fromKey.compareTo(currNode.key[i]) <= 0 
				&& toKey.compareTo(currNode.key[i]) > 0) {
					subMap.put(currNode.key[i], (V) currNode.ref[i]);
				}
			}
    ...
    private Node insert (K key, V ref, Node n)
    {
        ...
        if (n.isLeaf) {                                                      // handle leaf node level
            ...
            if(hasSplit) { // There's an unhandled split from below
            	Node leftChild = (Node) n.ref[i];
            	K dividerKey = leftChild.key[leftChild.nKeys - 1];
            	
            	// Use the divider key from previous split if possible
                if(!leftChild.isLeaf) {
                	leftChild.nKeys -= 1;
                }
            	
                if(n.nKeys < ORDER - 1) { // If the current node is not full                                                          
                    wedge(dividerKey, rt, n, i, false); 
                    hasSplit = false; // Update the split status
                } 
                
                else { // If the current node is full
                    if(DEBUG) out.println ("insert: internal node split");
                    
                    // Split current node: returns the right sibling
                    rt = split(dividerKey, rt, n, false);
